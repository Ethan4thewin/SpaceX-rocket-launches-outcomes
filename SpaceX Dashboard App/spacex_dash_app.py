...
def get_scatter(site, payload):
    min, max = payload
    print(min, max)
    cond = (spacex_df["Payload Mass (kg)"] > min) & (spacex_df["Payload Mass (kg)"] < max)
    if site == 'ALL':
        df = spacex_df[cond]
        fig = px.scatter(data_frame=df, x="Payload Mass (kg)",
            y="class",
            color="Booster Version Category",
            title="Correlation between Payload and Success for all sites")
        return fig
    else:
        df = spacex_df[spacex_df["Launch Site"] == site][cond]
        fig = px.scatter(data_frame=df, x="Payload Mass (kg)",
            y="class",
            color="Booster Version Category",
